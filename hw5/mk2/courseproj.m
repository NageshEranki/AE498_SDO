...
rho = 1*ones(str.Nx*str.Ny,1);
rho(esns.Eg) = 1e-3;
% rho=readmatrix('test.dat');

ep = 3*min([str.hx, str.hy]);
p = 3;

%   My own density filter
%w = topfilter(str.L,str.H,str.Nx,str.Ny,ep);

%   Prof's density filter
[x,y]=ndgrid(linspace(0,str.L,str.Nx+1),linspace(0,str.H,str.Ny+1));
w = density_filter( str.Nx , str.Ny , 0.5 , ...
    x(:) , y(:) , ...
    list_nodes(1:(str.Nx*str.Ny),str.Nx));
...
function [d,K,F] = femsolve(rho,p,str,esns)
    K = globalK( str.L , str.H , str.Nx , str.Ny , ...
        ...
        rho , p , ...
        str.E , str.nu );
    F = globalF_proj2( str.L , str.H , str.Nx , str.Ny , ...
         str.T , str.P0 );
...
%      tic
    ...
    %   of the reaction at the tip
    %   of the gripper surface
    f = -temp(esns.dofsg(end))/sum(abs(F));
%     toc
    %--------------------------------------------------------------------------
